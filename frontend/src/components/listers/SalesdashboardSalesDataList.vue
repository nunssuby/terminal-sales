...
<script>
    ...
    import SalesdashboardSalesData from './../SalesdashboardSalesData.vue';

    export default {
        name: 'SalesdashboardSalesDataManager',
        components: {
            SalesdashboardSalesData,
        },
        props: {
            offline: Boolean,
            editMode: Boolean,
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/salesdatas'))
            temp.data._embedded.salesdatas.map(obj => obj.id=obj._links.self.href.split("/")[obj._links.self.href.split("/").length - 1])
            this.values = temp.data._embedded.salesdatas;
